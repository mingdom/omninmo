...
try:
    ...
    from src.utils.model_utils import save_model_versioned, get_latest_model_path
...
def train_xgboost_model(tickers, model_path, period="5y", interval="1d", forward_days=20, force_sample=False):
    ...
    success, saved_path = save_model_versioned(
        predictor,
...
def train_on_default_tickers(model_path='models/stock_predictor.pkl', period='2y', tickers=None, force_sample=False):
    ...
    try:
        ...
        if model:
            logger.info(f"XGBoost model trained and saved successfully")
            return model
        else:
