...
def validate_option_data(
    ...
    try:
        ...
        raise DataError(f"Invalid quantity format '{quantity_raw}' for option '{description}': {e}")
    ...
    try:
        ...
        raise DataError(f"Invalid price format '{price_raw}' for option '{description}': {e}")
...
def validate_dataframe(
    ...
    if missing_columns:
        raise DataError(f"{name} is missing required columns: {', '.join(missing_columns)}")
...
def clean_numeric_value(
    ...
    try:
        ...
        if default is not None:
            return default
        raise ValueError(f"Could not convert '{value}' to a numeric value")
