...
def train_model(tickers=None, model_path=None, period=None, interval=None, 
               ...
               use_enhanced_features=True, use_risk_adjusted_target=True):
    """
    ...
    if use_enhanced_features:
        try:
            ...
            market_data = fetcher.fetch_market_data('SPY', period=period, interval=interval, force_sample=force_sample)
        except Exception as e:
            logger.warning(f"Failed to fetch market data: {e}. Beta features will not be available.")
    ...
    for ticker in tickers:
        try:
            ...
            df[future_return_col] = df['Close'].pct_change(forward_days).shift(-forward_days)
            
            # Generate features
            feature_df = features.generate(df, market_data, use_enhanced_features=use_enhanced_features)
            ...
            if len(feature_df) < 30:
                logger.warning(f"Not enough data after feature generation for {ticker}, skipping")
                skipped_tickers.append(ticker)
                continue
    ...
    logger.info(f"Combined dataset: {len(X)} rows from {len(processed_tickers)} tickers")
...
def main():
    ...
    parser.add_argument('--risk-adjusted-target', action='store_true',
