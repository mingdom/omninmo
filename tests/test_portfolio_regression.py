...
def mock_beta_calculation(monkeypatch):
    ...
    def mock_get_beta(ticker, description=""):
        ...
        if base_ticker.endswith(("C", "P")) and any(
            char.isdigit() for char in base_ticker
        ):
    ...
    monkeypatch.setattr(
        src.folio.portfolio, "is_cash_or_short_term", mock_is_cash_or_short_term
    )
...
def test_sample_portfolio_regression(sample_portfolio_path):
    ...
    ), "Summary is not a PortfolioSummary object"
    ...
    assert (
        summary.total_value_abs >= summary.total_value_net
    ), "Absolute value should be >= net value"

    # Test portfolio beta
    assert (
        0 <= summary.portfolio_beta <= 3.0
    ), "Portfolio beta should be in a reasonable range"

    # Test exposure breakdowns
    assert isinstance(
        summary.long_exposure, ExposureBreakdown
    ), "Long exposure is not an ExposureBreakdown object"
    assert isinstance(
        summary.short_exposure, ExposureBreakdown
    ), "Short exposure is not an ExposureBreakdown object"
    assert isinstance(
        summary.options_exposure, ExposureBreakdown
    ), "Options exposure is not an ExposureBreakdown object"

    # Test exposure values
    assert (
        summary.long_exposure.total_value > 0
    ), "Long exposure value should be positive"
    assert (
        summary.short_exposure.total_value >= 0
    ), "Short exposure value should be non-negative"
    ...
    assert (
        0 <= summary.short_percentage <= 100
    ), "Short percentage should be between 0 and 100"
    assert (
        0 <= summary.exposure_reduction_percentage <= 100
    ), "Exposure reduction should be between 0 and 100"
    ...
    assert (
        abs(
            summary.short_exposure.total_value - expected_values["short_exposure_value"]
        )
        / expected_values["short_exposure_value"]
        <= 0.1
    ...
    assert (
        abs(
            ...
            - expected_values["options_exposure_value"]
        )
        / expected_values["options_exposure_value"]
        <= 0.1
    ...
    assert (
        abs(summary.cash_like_value - expected_values["cash_like_value"])
        / expected_values["cash_like_value"]
        <= 0.1
...
def test_sample_portfolio_serialization(sample_portfolio_path):
    ...
    groups, summary, _ = process_portfolio_data(
        df
    )  # We don't need cash_like_positions for this test

    # Test serialization of summary
    summary_dict = summary.to_dict()
    assert isinstance(summary_dict, dict), "Summary should serialize to a dictionary"
    assert (
        "total_value_net" in summary_dict
    ), "Serialized summary missing total_value_net"
    ...
    for i, group in enumerate(groups):
        ...
        assert isinstance(
            group_dict, dict
        ), f"Group {i} should serialize to a dictionary"
        ...
        if group.stock_position:
            ...
            assert isinstance(
                stock_dict, dict
            ), f"Stock position in group {i} should serialize to a dictionary"
            assert "ticker" in stock_dict, f"Stock position in group {i} missing ticker"
            assert (
                "market_value" in stock_dict
            ), f"Stock position in group {i} missing market_value"
        ...
        for j, option in enumerate(group.option_positions):
            ...
            assert isinstance(
                option_dict, dict
            ), f"Option {j} in group {i} should serialize to a dictionary"
            ...
            assert (
                "option_type" in option_dict
            ), f"Option {j} in group {i} missing option_type"
